...
function runpattern(fsym, parnames, rules, combin)
	...
	for ic in 1:size(combin,1)  # try each arg dim in combin
		...
		for p in prange  # try each argument as parameter
			...
			f = Abcd.substSymbols(fsym, Dict(parnames, tpar))
			vn = symbol("arg$p")
...
@mtest testpattern2 logpdf(Normal(mu,sigma),x)  sigma->sigma<=0?0.1:sigma
deriv1(:(logpdf(Normal(1.,2.),x)), v1ref)  
deriv1(:(logpdf(Normal(1.,2.),x)), v2ref)  

# @mtest testpattern1 logpdfUniform(a,b,x)      a->a-10 b->b+10
@mtest testpattern2 logpdf(Uniform(a,b),x)    a->a-10 b->b+10
deriv1(:(logpdf(Uniform(-10,10),x)), v1ref)  
deriv1(:(logpdf(Uniform(-10,10),x)), v2ref)  

# @mtest testpattern1 logpdfGamma(sh,sc,x)      sh->sh<=0?0.1:sh  sc->sc<=0?0.1:sc  x->x<=0?0.1:x
@mtest testpattern2 logpdf(Gamma(sh,sc),x)      sh->sh<=0?0.1:sh  sc->sc<=0?0.1:sc  x->x<=0?0.1:x
deriv1(:(logpdf(Gamma(1.,2.),x)), abs(v1ref)+0.1)  
deriv1(:(logpdf(Gamma(1.,2.),x)), abs(v2ref)+0.1)  
